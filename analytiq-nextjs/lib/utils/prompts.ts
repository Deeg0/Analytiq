...
function getOptimizedContent(content: ExtractedContent): string {
  const maxLength = 18000; // Optimized to balance thoroughness with speed
  const parts: string[] = [];
  ...
  if (content.sections) {
    ...
    if (content.sections.abstract) {
      parts.push(`=== ABSTRACT ===\n${content.sections.abstract.substring(0, 2500)}\n`);
    }
    if (content.sections.introduction) {
      parts.push(`=== INTRODUCTION ===\n${content.sections.introduction.substring(0, 3500)}\n`);
    }
    if (content.sections.methods) {
      parts.push(`=== METHODS ===\n${content.sections.methods.substring(0, 4500)}\n`);
    }
    if (content.sections.results) {
      parts.push(`=== RESULTS ===\n${content.sections.results.substring(0, 4500)}\n`);
    }
    if (content.sections.discussion) {
      parts.push(`=== DISCUSSION ===\n${content.sections.discussion.substring(0, 3500)}\n`);
    }
